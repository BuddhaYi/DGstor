...
func (client *Client) TracePuts(args *interface{},reply *interface{}) error{
	...
	for objectId := uint64(client.ClientId); objectId < 10; objectId += uint64(client.NumClients) {
		ch <- objectId
	}
	close(ch)
	wg.Wait()
...
func (client *Client) TraceDget(args *interface{}, reply *interface{}) error {
	...
	for _, v := range DgetObjectId {
		...
		url := fmt.Sprintf("http://node1:%d/dget/%d", client.LocalHttpPort, v)
	...
	logrus.Println("total latency:", totalLatency, "ms.", "Total sizes:", totalSize>>20, "MB.")
	logrus.Println("Average latency:", totalLatency/float64(len(DgetObjectId)), "ms.")
	return nil
}
...
func (client *Client) TraceGet(args *interface{}, reply *interface{}) error {
	...
	for i:=0;i<sampleNum;i++{
		...
		url := fmt.Sprintf("http://node1:%d/get/%d",client.LocalHttpPort, oid)
