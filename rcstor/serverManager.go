...
func startStorageServerLocal(path string, constructor index.IndexConstructor, storage stor.StorageServiceInterface) uint16 {
	...
	//indexService := index.MakeNameLayer(path,constructor)
	err := s.RegisterName("IndexService", indexService)
	err = s.RegisterName("StorageService", storage)
