...
func (partitoner *GeometricPartitioner) Split(size uint64) []uint64 {
	...
	// Paper Algorithm 1: remainder is placed into the closest even-multiple container.
	// Pad remainder up to MinBlock to eliminate blockId=0 blocks and avoid hybrid coding.
	if size > 0 {
		list = append(list, partitoner.MinBlock)
	}
