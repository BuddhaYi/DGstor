...
//TODO:将索引根据所在磁盘进行反向索引，然后分片放置到不同的目录服务器节点上 对key进行哈希计算映射到固定的目录服务器上
type Metadata struct{
	Reverse_shard map[int16]*IndexService
}
func MakeNameLayer(workspace string, indexConstructor IndexConstructor) *NameLayer {
	service := new(NameLayer)
	service.Namenode = MakeIndexService(workspace,indexConstructor)

	service.disks = new(Metadata)
	service.disks.Reverse_shard = make(map[int16]*IndexService)
	return service
}
...
//func (service *NameLayer) PutIndex(args *PutIndexArgs, reply *ObjectIndex) error {
//	service.Namenode.mu.Lock()
//	defer service.Namenode.mu.Unlock()
//	pginfo := service.Namenode.getPGInfo(args.PGId)
//
//	if index, ok := pginfo.IndexMap[args.ObjectID]; ok {
//		//Update the object
//		if index.GetSize() == args.Size {
...
//
//
//	index := service.Namenode.indexConstructor.Construct(args,&pginfo.SpaceUsageInfo)
//	//service.indexConstructor.UpdateSpace(index,&pginfo.SpaceUsageInfo)
//
//	pginfo.IndexMap[args.ObjectID] = index
//	pginfo.IndexLogTail++
//	//根据反向索引将pginfotable存入
//	service.disks.Reverse_shard[index.Contiguous.IndexInPG] = service.Namenode
//	AddLog(args, service.Namenode.logDirPath(args.PGId), pginfo.IndexLogTail, pginfo, service.Namenode.logFdTable)
//
//	*reply = *index
//	return nil
//}
