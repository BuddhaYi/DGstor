...
// GeoPartitionService implements Geometric Partitioning (Geo) from reference [13].
// All blocks of an object are placed on the SAME brick (contiguous placement).
// Uses only global MSR encoding without local parity (DG-LRCs).
type GeoPartitionService struct {
...
func MakeGeoPartitionService(pool *dir.VolumeConnectionPool) *GeoPartitionService {
	service := &GeoPartitionService{}
	...
	service.encoder = encoder.MakeMSREncoder(service.k+service.redundancy, service.k, 0, service.bufPool)
...
func (service *GeoPartitionService) WriteResponse(w io.Writer, objectID uint64, offset uint64, size uint64, PG dir.PlacementGroup, objectIndex index.ObjectIndex, broken int) {
...
func (service *GeoPartitionService) RepairBlocks(blockId int, offset uint64, size uint64, pg dir.PlacementGroup, broken int, shuffled bool) []common.IOBuffer {
...
func (service *GeoPartitionService) RegenerateObject(PG dir.PlacementGroup, geoIndex *index.GeometricIndex, offset, size uint64, broken int) chan common.IOBuffer {
	var args stor.GetBatchDataArgs
	args.PGId = int(PG.PGId)
	...
	go func() {
		...
		for _, block := range geoIndex.Blocks {
			if block.BlockID == 0 {
				...
				repaired := service.RepairBlocks(0, block.OffsetInBucket, block.Size, PG, broken, true)
				...
				ret <- repaired[broken].Slice(start, end)
				currentOff = end
				continue
			}
...
func (service *GeoPartitionService) RegenerateBlock(blockId int, offset uint64, size uint64, broken int, PG dir.PlacementGroup) common.IOBuffer {
	n := service.k + service.redundancy
...
func (service *GeoPartitionService) Get(offset uint64, size uint64, PG dir.PlacementGroup, objectIndex index.ObjectIndex) chan common.IOBuffer {
...
func (service *GeoPartitionService) getMaxSize(blockID int16, pg dir.PlacementGroup) uint64 {
	k := service.k
	sizes := make([]uint64, k)
	var wg sync.WaitGroup
	for i := 0; i < k; i++ {
...
func (service *GeoPartitionService) GenerateParity(PG dir.PlacementGroup) {
	k := service.k
...
func (service *GeoPartitionService) Put(objectID uint64, data common.IOBuffer, PG dir.PlacementGroup, objectIndex index.ObjectIndex) {
	geoIndex := objectIndex.Geometric
	var wg sync.WaitGroup
...
func (service *GeoPartitionService) GetRecoverTasks(instruct dir.MoveInstruct) []dir.RecoveryTask {
	...
	for _, blockSize := range blockSizes {
		...
		logrus.Warnf("PG%d-block%d: %d", PG.PGId, blockId, toRecover)
...
func (service *GeoPartitionService) Recovery() {
	...
	for {
		...
		taskBlocks := int(task.Size / service.partitioner.MinBlock)
		for i := 0; i < taskBlocks; i++ {
			<-maxTasks
		}
		go func(task dir.RecoveryTask, taskBlocks int) {
			...
			go func(brick uuid.UUID, args *stor.PutDataArgs, data common.IOBuffer, taskBlocks int) {
				for i := 0; i < taskBlocks; i++ {
					<-maxWrites
				}
