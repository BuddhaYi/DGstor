...
func (service *DirectoryService) serveTasks() {
	for task := range service.tasks {
		...
		case "Recovery":
			logrus.Println("Begin to Recovery objectId:")
			start := time.Now()
			...
			logrus.Println("-----------")
			logrus.Println("Finish Recovery, latency:", latency)
		}
	}
}
...
func (service *DirectoryService) TraceGet(name *string, reply *interface{}) error {

	err := service.callClientsMethods(*name, "TraceGet",0)
	if err != nil {
		logrus.Errorln(err)
	}
	return err
