...
type VolumeParameter struct{
	...
	// LocalEncoderType specifies the local parity encoder: "MSR" (2 parity/group) or "RS" (1 parity/group)
	LocalEncoderType string `json:"LocalEncoderType"`

	IsSSD  bool  `json:"IsSSD"`
	HTTPPort int `json:"HTTPPort"`
}

// LocalParityPerGroup returns the number of local parity nodes per group based on encoder type.
func (p *VolumeParameter) LocalParityPerGroup() int {
	if p.LocalEncoderType == "RS" {
		return 1
	}
	return 2 // MSR default
}

// EffectiveLocalRedundancy returns the number of local groups, defaulting to 2.
func (p *VolumeParameter) EffectiveLocalRedundancy() int {
	if p.LocalRedundancy == 0 {
		return 2
	}
	return p.LocalRedundancy
}
