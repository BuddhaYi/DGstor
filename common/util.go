...
func (stats *TimeStats) AddDgetTime(dgetTime time.Duration){
	if stats.dgetTime == nil{
		stats.dgetTime = make([]time.Duration,0)
	}
	stats.dgetTime = append(stats.dgetTime,dgetTime)
}

func (stats *TimeStats) AddGetTime(getTime time.Duration){
	if stats.getTime == nil{
		stats.getTime = make([]time.Duration,0)
	}
	stats.getTime = append(stats.getTime,getTime)
}
...
func (stats *TimeStats) AverageDget() (dget int64) {
	for _,rep := range stats.dgetTime {
		dget += rep.Nanoseconds()
	}
	dget /= int64(len(stats.dgetTime))
	return
}

func (stats *TimeStats) AverageGet() (get int64) {
	for _,rep := range stats.getTime {
		get += rep.Nanoseconds()
	}
	get /= int64(len(stats.getTime))
	return
}
